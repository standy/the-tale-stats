...
//	};
	...
	/* загрузка словарей только для браузера */
	if (typeof window !== 'undefined' && window) window.setTimeout(prepare, 10);
	function prepare() {
		$.when(
			app.getJson('verbose'),
			app.getJson('players'),
...
//[ноя 03 16:45:08]
// настройки модулей
if (typeof window !== 'undefined' && window.moment && $.fn.customScroll && window.swig && window.CanvasJS) {
	moment.locale('ru');
...
app.on('ready', function() {
	...
	$('#theme-select').on('change', function() {
		var val = $(this).val();
		$('link[data-theme]').attr('href', 'static/css/themes/' + val);
	});
